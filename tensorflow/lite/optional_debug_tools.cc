...
SubgraphDelegationMetadata GetNodeDelegationMetadata(const Subgraph& subgraph) {
  ...
  metadata.replaced_node_bits.resize(subgraph.nodes_size());
  ...
  for (size_t node_index = 0; node_index < subgraph.nodes_size();
       node_index++) {
    metadata.replaced_node_bits[node_index] = false;
    ...
    if (delegate != nullptr) {
      ...
      for (int nid : TfLiteIntArrayView(params->nodes_to_replace)) {
        metadata.replaced_node_bits[nid] = true;
  ...
  for (int i = 0; i < num_subgraphs; ++i) {
    ...
    for (size_t node_index = 0; node_index < subgraph.nodes_size();
      ...
      if (node.delegate == nullptr) {
        if (delegation_metadata.replaced_node_bits[node_index]) {
