...
TfLiteStatus TfliteTensorToEdge(const TfLiteTensor* tensor, int tensor_index,
  ...
  const auto status = TfliteIntArrayToRepeatedField(
      tensor->dims, edge_proto.mutable_shape(), true);
...
TfLiteStatus TfliteNodeToNode(const TfLiteNode& node,
  ...
  status = TfliteIntArrayToRepeatedField(node.outputs,
                                         node_proto.mutable_outputs(), true);
  if (status != kTfLiteOk) {
    return status;
  }
  ...
  if (is_node_delegated) {
    ...
    status = TfliteIntArrayToRepeatedField(
        ...
        delegate_node_details->mutable_tflite_node_ids_replaced(), true);
    if (status != kTfLiteOk) {
      return status;
    }
  }
  ...
  for (int i = 0; i < num_subgraphs; ++i) {
    ...
    for (size_t node_index = 0; node_index < subgraph.nodes_size();
      ...
      const bool is_node_delegated =
          ...
          delegation_metadata.replaced_node_bits[node_index];
